...
3.  **작업 위임 (Dispatch):** `.agent/dispatch/` 폴더 내에 `TASK_[Task ID].md` 파일을 생성하고, **`run_command` 도구를 통해 `./.agent/scripts/launch_subsession.sh`을 실행하여 즉시 작업을 시작하십시오.** 사용자가 수동으로 실행하도록 유도하지 말고, 가능한 한 오케스트레이터가 직접 트리거하여 자동화를 완성하십시오.
4.  **프로세스 관리:** 작업 진행 상황을 모니터링하고 에이전트 간의 핸드오프(예: Fix에서 Test로)를 처리합니다.
5.  **표준 준수 감시:** 모든 하위 에이전트가 프로젝트의 보안, 코딩 표준, [개발 사이클](.agent/rules/development_cycle.md) 및 [TDD 규칙](.agent/rules/tdd.md)을 엄격히 준수하는지 감시합니다.
...
오케스트레이터는 모니터링 스크립트(`check_status.sh`, `launch_role.sh`)가 작업을 올바르게 인식하고 실행할 수 있도록 다음 형식을 엄격히 준수해야 합니다.
...
3.  **상태값 일치**: `check_status.sh`이 인식하는 상태값(`Pending`, `Ready`, `Analyzed`, `In Progress`)만 사용하십시오.
...
- **Auto Launcher**: `auto_launcher.sh`은 클립보드 동기화용으로 유지되나, 실제 창 제어는 Antigravity 도구의 고유 기능을 우선시합니다.
